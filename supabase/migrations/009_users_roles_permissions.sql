-- =============================================================================
-- 009: Users, Roles, Permissions
--
-- Substitui a migration 008 (mantida no repo só como histórico — não rode as
-- duas). Mesmo conteúdo do 008, já com os dois bugs corrigidos e confirmados
-- contra o schema real do Supabase:
--   - removida a comparação impossível `p.role = 'moderador'` (o enum
--     public.role_type no banco real só tem 'admin'/'supplier')
--   - removida a referência a `p.must_change_password` (essa coluna nunca
--     existiu em profiles no banco real; a migration 007 que a criaria não
--     foi aplicada)
--
