import "server-only";
import { BadRequestException, NotFoundException } from "@/server/http";
import { adminClient } from "@/server/supabase";
import type { CotacaoStatus, PropostaStatus } from "../cotacoes/cotacoes.types";
import type { ConviteStatus } from "../propostas/fornecedor-token.service";
 ...
 * `null`. Aceita as duas formas para não voltar a quebrar se a cardinalidade
 * mudar.
 */
 ...
 * them — there is no FK between fornecedores_convidados and users, so this
 * matches by email OR whatsapp instead (convites criados pelo modal de
 * WhatsApp podem não ter e-mail preenchido).
 */
async function matchingConvidadoIds(email: string, whatsapp?: string | null): Promise<string[]> {
  const supabase = adminClient();
  const ids = new Set<string>();

  const { data: porEmail, error: emailError } = await supabase
  ...
  if (whatsapp) {
    ...
    const variantes = [whatsapp, normalizarWhatsapp(whatsapp)].filter((v): v is string => !!v);

    const { data: porWhatsapp, error: whatsappError } = await supabase
      .from("fornecedores_convidados")
      .select("id")
...
export async function listarMeusConvites(
  email: string,
  whatsapp?: string | null,
): Promise<MeuConvite[]> {
  const matchedIds = await matchingConvidadoIds(email, whatsapp);
  ...
  // a proposta ligada a este convite pra dar ao front um jeito de linkar
  // direto pra Minhas Propostas.
  ...
  // Ordem padrão: abertas primeiro, depois as mais recentes.
...
/**
 * Propostas enviadas pela conta logada — mesma ponte por e-mail OU whatsapp
 * de listarMeusConvites (propostas.fornecedor_id não existe no schema real).
 */
export async function listarMinhasPropostas(
  email: string,
  whatsapp?: string | null,
): Promise<MinhaProposta[]> {
  const matchedIds = await matchingConvidadoIds(email, whatsapp);
  if (matchedIds.length === 0) return [];
...
/**
 * Detalhe de uma proposta enviada (itens + dados da cotação). Valida a posse
 * via e-mail ou whatsapp pra que o fornecedor só veja propostas que ele
 * mesmo enviou.
 */
export async function detalharMinhaProposta(
  email: string,
  propostaId: string,
  whatsapp?: string | null,
