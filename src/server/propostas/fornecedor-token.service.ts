import "server-only";
import { BadRequestException, ConflictException, NotFoundException } from "@/server/http";
 ...
 * módulo é o único lugar que resolve os dois.
 */

export async function validateToken(token: string): Promise<FornecedorConvidado> {
 ...
 */
export async function resolveConvite(identificador: string): Promise<FornecedorConvidado> {
...
export async function markVisualizado(convidado: FornecedorConvidado): Promise<void> {
...
export async function validateTokenForSubmission(token: string): Promise<FornecedorConvidado> {
...
export async function markRespondido(
...
export async function vincularUsuario(
  ...
  const pertenceAoUsuario = (linha: FornecedorConvidado): boolean => {
    ...
    const mesmoEmail =
      !!emailReal &&
      !isEmailSentinela(linha.email_contato) &&
  ...
  const { data: daCotacao, error } = await supabase
    .from("fornecedores_convidados")
    .select(CONVIDADO_COLUNAS)
